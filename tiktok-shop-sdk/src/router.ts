...
export function registerRoutes(
  ...
  app.post<{ Body: ProxyRequestBody }>("/api/shop/proxy", async (req, reply) => {
    const requestId = uuidv4();
    const { method, path, access_token, shop_cipher, query_params, body: reqBody } =
      req.body;
    ...
    if (!API_PATH_PATTERN.test(apiPath)) {
      reply.status(400).send({
        success: false,
        error: { code: "INVALID_PATH", message: "path does not match expected API format" },
        request_id: requestId,
      });
      return;
    }
    ...
    // Serialize body
    const bodyStr = reqBody && Object.keys(reqBody).length > 0
      ? JSON.stringify(reqBody)
      : "";

    // Generate signature
    allParams["sign"] = generateSignature(apiPath, allParams, bodyStr, appSecret);
