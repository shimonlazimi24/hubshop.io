...
class TestShopHMACSigning:
    def test_signature_generation(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "backend.tiktok.shop.client.settings.tiktok_shop_app_key", "app123"
        )
        monkeypatch.setattr(
            "backend.tiktok.shop.client.settings.tiktok_shop_app_secret", "secret456"
        )
    ...
    def test_signature_excludes_sign_and_access_token(
    ...
    def test_signature_includes_body(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        sig_with_body = client._generate_signature(
            "/api/test", params, '{"key":"value"}'
        )
    ...
    def test_signature_params_sorted(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert client._generate_signature(
            "/api/test", params_a, ""
        ) == client._generate_signature("/api/test", params_b, "")
